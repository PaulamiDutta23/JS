...
function areDeepEqual(array1, array2) {
  if (array1.length !== array2.length || isNotEqualType(array1, array2)) {
    return false;
  }
  for (let index = 0; index < array1.length; index++) {
    if (isArray(array1[index])) {
      if(!areSubarrayEqual(array1[index], array2[index])) {
        return false;
      }
    }
...
function testAllAreDeepEqual() {
  ...
  testAreDeepEqual("string passed as argument", "abc", ['a', 'b', 'c'], false);
  testAreDeepEqual("2 subarrays and unequal", [[], [1]], [[1],[]], false);
}
